...
<div class="col-sm-5th col-sm-5">
    <div class="box" style="padding:15px 5px;">
        ...
        <?php
        $img = explode(',', $model->view->images);
        ?>
        <a href="#<?= $model->view->slug ?>" class="item-modal" data-id="<?= $model->view->id ?>">
        ...
        <div class="addcart">
            <ul class="list-inline">
                <li><?= number_format($model->view->price, 0, '', '.') ?> VND</li>
                <li class="pull-right num-cart">
                    <a href="javascript:void(0)" class="btn btn-danger add add_<?= $model->id ?>" data="0"  style="display: <?= !empty($_SESSION['cart'][$model->view->id]) ? "none" : "block" ?>">
                        Thêm
                    </a>
                    <div class="empty empty_<?= $model->id ?>" style="display: <?= !empty($_SESSION['cart'][$model->view->id]) ? "block" : "none" ?>">
                        <span class="btn btn-danger addto" data="1"><i class="md md-remove"></i></span>
                        <span class="quantity"><?php
                            if (!empty($_SESSION['cart'][$model->view->id]['quantity']))
                                echo $_SESSION['cart'][$model->view->id]['quantity']
                                ?></span>

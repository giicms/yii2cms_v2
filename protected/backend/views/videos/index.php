...
$("input[name=status]").on("switchChange.bootstrapSwitch", function(event, state) {
    var key = $(this).parent().parent().parent().parent("tr").attr("data-key");
        $.ajax({type: "POST", url:"' . Yii::$app->urlManager->createUrl(["video/status"]) . '", data: {id: key,state:state}, success: function (data) {
            }, });
});
') ?>
<?= $this->registerJs('
    ...
    if (msg == true) {
     if(keys!=""){
      $.ajax({
                type: "POST",
                url:"' . Yii::$app->urlManager->createUrl(["video/deletes"]) . '", data: {ids: keys,page:$("#page").val()}, success: function (data) {
                },    
            });
         }
    }
...
<?= $this->registerJs('
    $(".status").click(function () {
      ...
      $.ajax({
                type: "POST",
                url:"' . Yii::$app->urlManager->createUrl(["video/status"]) . '", data: {ids: keys,page:$("#page").val()}, success: function (data) {
                    if(data=="ok"){
